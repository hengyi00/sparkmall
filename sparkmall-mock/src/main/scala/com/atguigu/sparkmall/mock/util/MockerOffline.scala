...
import com.atguigu.sparkmall.common.model.{CityInfo, ProductInfo, UserInfo, UserVisitAction}
...
object MockerOffline {
    ...
    def insertHive(sparkSession: SparkSession,tableName:String,dataFrame:DataFrame): Unit ={
        sparkSession.sql("use " + ConfigUtil.getValueByKey("hive.database"))
    ...
    def mockUserAction()={
        ...
        val startDate = new   SimpleDateFormat("yyyy-MM-dd").parse("2019-07-14")
        val endDate = new   SimpleDateFormat("yyyy-MM-dd").parse("2019-07-15")
